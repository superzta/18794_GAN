...
from model_variants import DCGenerator, DCDiscriminator
...
def training_loop(train_dataloader, opts):
    ...
    for _ in range(opts.num_epochs):
        ...
        for batch in train_dataloader:
            ...
            # D_real_loss = mse_loss(D(real_images), torch.ones_like(D(real_images)))

            real_targets = torch.ones_like(D(real_images))
            D_real_loss = 0.5 * mse_loss(D(real_images), real_targets)

            # 2. Sample noise
            noise = sample_noise(current_batch_size, opts.noise_size)

            # 3. Generate fake images from the noise
            fake_images = G(noise)

            # 4. Compute the discriminator loss on the fake images
            fake_targets = torch.zeros_like(D(fake_images.detach()))
            D_fake_loss = 0.5 * mse_loss(D(fake_images.detach()), fake_targets)
            ...
            gen_targets = torch.ones_like(D(fake_images))
            G_loss = 0.5 * mse_loss(D(fake_images), gen_targets)
